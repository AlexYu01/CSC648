    ...
    <nav id="team5-menu" class="team5-menu" lang="en" style="position:fixed;right:0;left:0;z-index:1;">
        ...
        <div id="wm-user-component">
            ...
            if ( $this->request->session()->read( 'Auth' ) ) {
                ?>
                <a href="<?= $this->url->build( ['controller' => 'Media', 'action' => 'posts'] ) ?>">
                    <span style="margin:-50px 0px 0px 40px; border: 0px; font-size:1.5em; float:left;"> Welcome, <?= $this->request->session()->read( 'Auth.User.username' ) ?>
                    </span></a>
                <?php
                echo $this->Html->link( 'Logout', ['controller' => 'Users', 'action' => 'logout'], [
                    'class' => 'signin', 'target' => '_self'] );
                echo '<a class="signin" style="border:0px;margin-top:-5px;" href=' . $this->Url->build( [
                    'controller' => 'Messages', 'action' => 'received_msgs'] ) . '><span class="fa-layers fa-fw" style="font-size: 2em;">
