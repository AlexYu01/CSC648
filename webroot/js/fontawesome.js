...
var ATTRIBUTES_WATCHED_FOR_MUTATION = ['data-prefix', 'data-icon', 'data-fa-transform'];

var RESERVED_CLASSES = ['lg', '2x', '3x', '4x', '5x', 'fw', 'ul', 'li', 'border', 'pull-left', 'pull-right', 'spin', 'pulse', 'rotate-90', 'rotate-180', 'rotate-270', 'flip-horizontal', 'flip-vertical', 'stack', 'stack-1x', 'stack-2x', 'inverse', 'layers', 'layers-text', 'layers-counter'].concat(oneToTwenty.map(function (n) {
  return 'w-' + n;
}));
...
function makeInlineSvg(params) {
  ...
  var attributes = index$1({}, extra.attributes, {
    'data-prefix': prefix,
    'data-icon': iconName,
    'class': [config.replacementClass, config.familyPrefix + '-' + iconName, widthClass].concat(extra.classes).join(' '),
    'role': 'img',
    'xmlns': 'http://www.w3.org/2000/svg',
    'viewBox': '0 0 ' + width + ' ' + height
  });

  var styles = index$1({}, extra.styles);
  ...
  var titleTag = title ? '<title id="' + attributes['aria-labelledby'] + '">' + htmlEscape(title) + '</title>' : '';

  var out = '<svg ' + joinAttributes(attributes) + '>' + titleTag + ('<path d="' + layers[0] + '" />') + '</svg>';
...
function makeLayersText(params) {
  ...
  return '<span ' + joinAttributes(attributes) + '>' + content + '</span>';
}
...
var build = function build() {
  _byUnicode = mapValues(packs$2, function (pack) {
  ...
  _byLigature = mapValues(packs$2, function (pack) {
    return reduce(pack, function (acc, icon, iconName) {
...
function classParser(node) {
  ...
  var val = toArray(node.classList).reduce(function (acc, cls) {
  ...
  if (style) {
    val = style.split(';').reduce(function (acc, style) {
      var _style$split = style.split(':'),
          _style$split2 = toArray$1(_style$split),
...
function transformParser(transformString) {
  var transform = {
    size: 16,
    x: 0,
    y: 0,
...
function parseMeta(node) {
  ...
  var transform = transformParser(node.getAttribute('data-fa-transform'));

  var finalName = iconName;

  if (prefix && node.innerText !== undefined && node.innerText.length > 1) {
    finalName = byLigature(prefix, node.innerText);
  } else if (prefix && node.innerText !== undefined && node.innerText.length === 1) {
    finalName = byUnicode(prefix, toHex(node.innerText));
  }
  ...
  if (config.autoA11y && title) {
    extraAttributes['aria-labelledby'] = config.replacementClass + '-' + iconName + '-title-' + nextUniqueId();
  }
...
function generateSvgReplacementMutation(node, nodeMeta) {
  ...
  if (iconName && prefix && packs[prefix] && packs[prefix][iconName]) {
    ...
    var _icon = toArray$1(icon),
        width = _icon[0],
        height = _icon[1],
        _ligatures = _icon[2],
        _unicode = _icon[3],
        layers = _icon.slice(4); // eslint-disable-line no-unused-vars


    val = [node, makeInlineSvg({
      prefix: prefix,
      iconName: iconName,
      width: width,
      height: height,
...
namespace.hooks = index$1({}, namespace.hooks, {
  addPack: function addPack(prefix, icons) {
    namespace.packs[prefix] = index$1({}, namespace.packs[prefix] || {}, icons);
